RUNTIME('getTabErrors', null, function(response) {
    if (!response.tabError || response.tabError.length === 0) {
        ...
        setSanitizedContent(document.querySelector('#main-message .error-code'), tabError.error);
        var a = document.querySelector('#main-message a');
        setSanitizedContent(a, tabError.url);
        a.setAttribute('href', tabError.url);
        var host = tabError.url.replace(/^\w+:\/\/([^\/]+)\/.*$/i, "$1");
        if (/.*\..*\./.test(host)) {
            host = host.replace(/^[^\.]*\./, '');
        }
        setSanitizedContent(document.getElementById('errorHost'), host);
