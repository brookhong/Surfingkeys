var KeyboardUtils = (function() {
    ...
    self.getKeyChar = function(event) {
        var character;
    ...
    self.isWordChar = function(event) {
    ...
    function _encodeKeystroke(s, k) {
        ...
        var code;
        if (k.length > 1) {
            code = 256 + self.specialKeys.indexOf(k);
        } else {
            code = k.charCodeAt(0);
        }
    ...
    self.encodeKeystroke = function (s) {
    ...
    self.specialKeys = ['Esc', 'Space', 'F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F7', 'F8', 'F9', 'F10', 'F11', 'F12', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Backspace', 'Enter', 'Tab', 'Delete', 'End', 'Home', 'Insert', 'NumLock', 'PageDown', 'PageUp', 'Pause', 'ScrollLock', 'CapsLock', 'PrintScreen', 'Escape', 'Hyper'];

    self.decodeKeystroke = function (s) {
        var ret = "";
