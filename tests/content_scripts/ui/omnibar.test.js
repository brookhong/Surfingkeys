...
describe('ui omnibar', () => {
    ...
    beforeAll(async () => {
        ...
        window.postMessage({ action: "initFrontend", origin: document.location.origin }, document.location.origin);
    ...
    test("toggle Omnibar's position", async () => {
        ...
        window.postMessage({ action: "openOmnibar", type: "URLs", extra: "getAllSites" }, document.location.origin);
        await new Promise((r) => setTimeout(r, 100));
